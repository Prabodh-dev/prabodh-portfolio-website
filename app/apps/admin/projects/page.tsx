...
export default function ProjectsAdmin() {
  ...
  const handleSave = async () => {
    ...
    if (!title || !summary || !description) {
      setMessage("Title, summary, and description are required");
      return;
    }
    ...
    <div className="min-h-screen admin-bg">
      <header className="bg-retro-darker border-b-2 border-retro-purple text-retro-cyan p-4">
        <div className="max-w-7xl mx-auto flex justify-between items-center">
          <Link
            href="/apps/admin"
            className="text-lg font-mono hover:text-retro-pink transition-colors"
          >
            {"<"} BACK_TO_DASHBOARD
          </Link>
          <h1 className="text-2xl font-mono font-bold uppercase neon-text">
            {">"} MANAGE_PROJECTS
          </h1>
        </div>
      </header>
        ...
        {!editingProject ? (
            ...
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              {projects.map((project) => (
                <div key={project.id} className="retro-card">
                  ...
                  <p className="text-sm text-retro-cyan mb-4 font-mono">
                    {project.summary}
                  </p>
                  <div className="flex gap-2 mb-3 flex-wrap">
                    {project.featured && (
                      <span className="text-xs px-2 py-1 bg-retro-pink text-retro-dark font-mono">
                        FEATURED
                      </span>
                    )}
          ...
          <div className="neon-box bg-retro-darker/50 p-8">
            ...
            <div className="space-y-6">
              <div>
                <label className="block text-retro-purple font-mono mb-2 uppercase tracking-wider">
                  {">"} TITLE *
                </label>
                <input
                  type="text"
              ...
              <div>
                <label className="flex items-center gap-2">
                  ...
                  />
                  <span className="text-retro-cyan font-mono uppercase">
                    FEATURED PROJECT
                  </span>
                </label>
              </div>
              ...
              {message && (
                <p
                  className={`font-mono uppercase ${message.toLowerCase().includes("success") ? "text-retro-green" : "text-red-500"}`}
                >
